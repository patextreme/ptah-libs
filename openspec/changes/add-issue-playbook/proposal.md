...
- One operation, `pickUp`:
  ...
  - **claim**: a bare `<!-- ptah:issue-claim -->` marker comment on the
    ...
    earliest claim comment (by gh `createdAt`, comment id as tie-break)
    wins; a losing runner backs off and takes the next eligible issue;
  - returns a typed **brief**: `{ number, url, title, body,
    claimCommentId }` — the consumer's script drives the work from it.
