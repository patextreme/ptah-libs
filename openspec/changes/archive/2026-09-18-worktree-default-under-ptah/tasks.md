...
- [x] 1.4 Prune a stale registration (a registered worktree whose directory was deleted out-of-band — `git clean -ffdx`, a manual `rm`) before the adopt check, so adoption never returns a nonexistent path; the worktree is re-created at the same path from the surviving branch, and a stale registration that survives the prune (a locked worktree) raises instead of being adopted or unlocked (D5) — verify in the harness by running the forced clean while the worktree is still registered (teardown moved after it; the registration's survival is asserted before the re-provision, so the case fails against the pre-self-heal module), plus a locked case that expects the raise
