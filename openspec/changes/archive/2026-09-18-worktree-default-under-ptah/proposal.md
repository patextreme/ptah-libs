...
- `provision` prunes a stale registration (a registered worktree whose directory was deleted out-of-band, e.g. by `git clean -ffdx`) and re-creates the worktree at the same path from the surviving branch — adoption never returns a nonexistent path.
